...
local function setup_autocaps()
  -- Prevent attaching twice to the same buffer
  ...
  local function map_char(char, replacement)
    vim.keymap.set("i", char, function()
      -- 1. Check if after sentence-ending punctuation (e.g., ". ")
      -- \v = very magic, [.!?] = punctuation, \s+ = whitespace, %# = cursor pos
      -- bcnw = backward, accept match at cursor, do not move cursor, don't wrap
      if vim.fn.search([[\v[.!?]\s+%#]], "bcnw") ~= 0 then
        return replacement
      end
      -- 2. Check if at the very start of a line (ignoring whitespace)
      ...
      -- 3. Default: return original character
      return char
    end, { expr = true, buffer = true })
  end

  -- Loop a-z to map them to A-Z
...
-- Create the Auto-Command to trigger the logic
vim.api.nvim_create_autocmd("FileType", {
  pattern = { "text", "markdown", "tex" }, -- Added "tex" for LaTeX support
  callback = setup_autocaps,
})
